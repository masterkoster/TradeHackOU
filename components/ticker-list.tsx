...
export function TickerList() {
  return (
    <section className="grid gap-4 md:grid-cols-3">
      {rows.map((row) => (
        <div key={row.title} className="rounded-xl border border-black/10 bg-white p-5 shadow-sm">
          <h3 className="text-base font-semibold text-black">{row.title}</h3>
          <p className="mt-2 text-sm text-black/60">{row.detail}</p>
        </div>
      ))}
    </section>
  )
