  ...
  if (mode === 'choose') {
    return (
      <div className="max-w-xl">
        <h1 className="text-lg font-semibold text-white mb-2">Risk Profile</h1>
        <p className="text-sm text-white/40 mb-8">
          Your risk profile controls how aggressively the signal engine reacts to sentiment data.
          {riskProfile && (
            <span className="ml-2 text-white/60">
              Current: <span className="capitalize font-medium text-white/80">{riskProfile}</span>
            </span>
          )}
        </p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <button
            ...
            className="text-left p-5 rounded-xl border border-white/10 bg-[#0D0D0D] hover:border-white/25 hover:bg-white/5 transition-all group"
          >
            ...
            <p className="text-sm font-medium text-white mb-1">I know my style</p>
          ...
          >
            ...
            <p className="text-sm font-medium text-white mb-1">Take the quiz</p>
            <p className="text-xs text-white/40 leading-relaxed">
    ...
    return (
      <div className="max-w-xl">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={backToChoose} className="text-xs text-white/40 hover:text-white/70 transition-colors">
            ← Back
          </button>
          <h1 className="text-lg font-semibold text-white">Choose Risk Profile</h1>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className={`font-medium text-sm mb-1 ${isActive ? profile.color : 'text-white'}`}>
                      {profile.label}
                    </p>
                    <p className="text-xs text-white/50 leading-relaxed">{profile.description}</p>
                  </div>
                  <div className={`w-4 h-4 rounded-full border-2 shrink-0 mt-0.5 flex items-center justify-center ${
                    isActive ? profile.borderActive : 'border-white/20'
                  }`}>
        ...
        {/* Threshold preview */}
        <div className="p-5 rounded-xl bg-[#0D0D0D] border border-white/10 mb-6">
          <p className="text-xs text-white/40 uppercase tracking-wide mb-4">
            Signal Thresholds — {active.label}
          </p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs text-white/40 mb-1">BUY trigger</p>
              <p className="text-2xl font-bold text-green-400">{active.buyThreshold}%</p>
              <p className="text-xs text-white/30 mt-1">positive sentiment confidence</p>
            </div>
            <div>
              <p className="text-xs text-white/40 mb-1">SELL trigger</p>
              <p className="text-2xl font-bold text-red-400">{active.sellThreshold}%</p>
              <p className="text-xs text-white/30 mt-1">negative sentiment confidence</p>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="max-w-xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-lg font-semibold text-white">Risk Quiz</h1>
        </div>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <span className="text-xs text-white/40 shrink-0">{quizStep + 1} / {QUIZ_QUESTIONS.length}</span>
        </div>

        <p className="text-base font-medium text-white mb-6 leading-snug">{q.scenario}</p>
        ...
        <div className="flex flex-col gap-3">
          {q.answers.map((answer, i) => (
            <button
              ...
              className="text-left p-4 rounded-xl border border-white/10 bg-[#0D0D0D] hover:border-[#22c55e]/50 hover:bg-[#22c55e]/5 text-sm text-white/80 hover:text-white transition-all"
            >
              <span className="text-white/30 mr-3 font-mono text-xs">{String.fromCharCode(65 + i)}</span>
              {answer.label}
            </button>
          ))}
        </div>
  ...
  return (
    <div className="max-w-xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-lg font-semibold text-white">Profile Saved</h1>
      </div>
      ...
      <div className={`p-6 rounded-xl border ${saved.bgActive} ${saved.borderActive} mb-6`}>
        ...
        <p className="text-xs text-white/50 leading-relaxed mb-5">{saved.description}</p>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs text-white/40 mb-1">BUY trigger</p>
            <p className="text-2xl font-bold text-green-400">{saved.buyThreshold}%</p>
            <p className="text-xs text-white/30 mt-1">positive confidence</p>
          </div>
          <div>
            <p className="text-xs text-white/40 mb-1">SELL trigger</p>
            <p className="text-2xl font-bold text-red-400">{saved.sellThreshold}%</p>
            <p className="text-xs text-white/30 mt-1">negative confidence</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <button
          ...
          className="flex-1 py-2.5 rounded-xl border border-white/10 text-sm text-white/60 hover:text-white hover:border-white/25 transition-colors"
        >
          Change Profile
        </button>
        {quizResult && (
          <button
            onClick={resetQuiz}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 text-sm text-white/60 hover:text-white hover:border-white/25 transition-colors"
          >
            <RotateCcw size={13} />
            Retake Quiz
          </button>
